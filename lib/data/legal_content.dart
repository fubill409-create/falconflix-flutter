...
const Map<String, LegalDocData> _terms = {
  'en': LegalDocData(
    ...
    sections: [
      ...
      [
        ...
        'We have zero tolerance for objectionable content and abusive behaviour. Pornographic, violent, hateful, harassing or otherwise objectionable user-generated content, and users who engage in such behaviour, are strictly prohibited. You can report objectionable content and block abusive users directly in the app using the built-in “Report” and “Block” tools. We review every report and will act within 24 hours — removing the offending content and/or banning the responsible user.',
      ],
      [
  ...
  'zh': LegalDocData(
    ...
    sections: [
      ...
      [
        ...
        '我们对令人反感的内容及滥用行为采取“零容忍”政策。严禁发布色情、暴力、仇恨、骚扰或其他令人反感的用户生成内容，亦严禁任何实施上述行为的用户。您可通过 App 内置的“举报”和“拉黑”功能，直接举报令人反感的内容并拉黑滥用行为的用户。我们会审核每一条举报，并在收到举报后 24 小时内处置——移除违规内容并/或封禁责任用户。',
      ],
      [
        '五、知识产权',
  ...
  'ja': LegalDocData(
    ...
    sections: [
      ...
      ['4. 利用者の行為規範', '法令や公序良俗に反する行為（違法・権利侵害コンテンツの投稿、マルウェアの拡散、無断録画・盗録、リバースエンジニアリングや妨害、不正な手段によるコイン・他者情報の取得を含むがこれらに限らない）に本サービスを利用しないことに同意します。違反した場合、当社は機能制限・凍結・アカウント抹消等の措置をとり、法的責任を追及する権利を留保します。', '当社は不快・不適切なコンテンツおよび迷惑行為に対して一切許容しません。わいせつ・暴力的・差別的・嫌がらせその他不快なユーザー投稿コンテンツ、ならびにそうした行為を行う利用者を固く禁止します。アプリ内蔵の「通報」および「ブロック」機能を使って、不快なコンテンツの通報や迷惑な利用者のブロックを直接行えます。当社はすべての通報を確認し、受領後 24 時間以内に対応します——該当コンテンツの削除および／または該当利用者の利用停止を行います。'],
  ...
  'ko': LegalDocData(
    ...
    sections: [
      ...
      ['4. 이용자 행위 규범', '법령이나 공서양속에 반하는 행위(불법·침해 콘텐츠 게시, 악성 프로그램 유포, 무단 녹화·복제, 리버스 엔지니어링이나 방해, 부정한 수단으로 코인·타인 정보 취득 등을 포함하되 이에 한정되지 않음)에 서비스를 이용하지 않을 것에 동의합니다. 위반 시 회사는 기능 제한·동결·계정 말소 등의 조치를 취하고 법적 책임을 물을 권리를 보유합니다.', '회사는 불쾌감을 주는 콘텐츠와 악용 행위에 대해 "무관용" 정책을 적용합니다. 음란·폭력·혐오·괴롭힘 등 불쾌감을 주는 이용자 생성 콘텐츠 및 그러한 행위를 하는 이용자를 엄격히 금지합니다. 이용자는 앱에 내장된 "신고" 및 "차단" 기능으로 불쾌한 콘텐츠를 신고하고 악용 이용자를 직접 차단할 수 있습니다. 회사는 모든 신고를 검토하고 접수 후 24시간 이내에 조치합니다 — 해당 콘텐츠 삭제 및/또는 책임 이용자의 이용 정지를 시행합니다.'],
  ...
  'ar': LegalDocData(
    ...
    sections: [
      ...
      ['٤. سلوك المستخدم', 'توافق على استخدام الخدمة بشكل قانوني وعدم استخدامها في أي نشاط يخالف القوانين المعمول بها أو النظام العام والآداب، بما في ذلك على سبيل المثال لا الحصر: نشر محتوى غير قانوني أو منتهِك؛ نشر البرمجيات الخبيثة؛ تسجيل أو نسخ محتوى الخدمة دون إذن؛ الهندسة العكسية أو تعطيل الخدمة؛ أو الحصول غير المشروع على العملات أو معلومات حسابات الآخرين. وعند مخالفتك قد نقيّد الميزات أو نوقف الحساب أو نغلقه، ونحتفظ بحق اتخاذ الإجراءات القانونية.', 'نطبّق سياسة «عدم التسامح المطلق» تجاه المحتوى المُسيء والسلوك التعسّفي. يُمنع منعًا باتًا نشر محتوى المستخدمين المُسيء — كالمحتوى الإباحي أو العنيف أو المُحرّض على الكراهية أو المتضمّن تحرّشًا أو غير ذلك من المحتوى المُسيء — كما يُمنع المستخدمون الذين يأتون بمثل هذا السلوك. ويمكنك الإبلاغ عن المحتوى المُسيء وحظر المستخدمين المتعسّفين مباشرةً داخل التطبيق عبر أداتي «الإبلاغ» و«الحظر» المدمجتين. ونحن نراجع كل بلاغ ونتّخذ الإجراء خلال ٢٤ ساعة — بإزالة المحتوى المخالف و/أو حظر المستخدم المسؤول.'],
  ...
  'fr': LegalDocData(
    ...
    sections: [
      ...
      ['4. Conduite de l’utilisateur', 'Vous vous engagez à utiliser le Service licitement et à ne pas l’employer pour une activité contraire aux lois applicables ou à l’ordre public, y compris, sans s’y limiter : publier des contenus illicites ou contrefaisants ; diffuser des logiciels malveillants ; enregistrer ou copier des contenus du Service sans autorisation ; faire de l’ingénierie inverse ou perturber le Service ; ou obtenir illicitement des Eagle Coins ou les informations de comptes d’autrui. En cas de manquement, nous pouvons restreindre des fonctions, suspendre ou fermer le compte, et nous réservons le droit d’agir en justice.', 'Nous appliquons une tolérance zéro envers les contenus répréhensibles et les comportements abusifs. Les contenus générés par les utilisateurs à caractère pornographique, violent, haineux, harcelant ou autrement répréhensible, ainsi que les utilisateurs qui adoptent de tels comportements, sont strictement interdits. Vous pouvez signaler les contenus répréhensibles et bloquer les utilisateurs abusifs directement dans l’application au moyen des outils intégrés « Signaler » et « Bloquer ». Nous examinons chaque signalement et agissons dans un délai de 24 heures — en supprimant le contenu en cause et/ou en bannissant l’utilisateur responsable.'],
